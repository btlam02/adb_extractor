...
float fieldOfView(mat4 projectionMatrix)
{
    float fov = 2.0 * atan(1.0 / projectionMatrix[1][1]);
    return fov;
}
float calculateZoomLevel(float distance, float screenHeight, mat4 projectionMatrix)
{
    float fov = fieldOfView(projectionMatrix);
    float halfFov = fov / 2.0;
    float viewSizeFactor = tan(halfFov) * 2.0;
    float verticalViewSizeMeters = distance * viewSizeFactor;
    float metersPerPixel = verticalViewSizeMeters / screenHeight;
    float kEarthTileSize = 40075016.0;
    float pixels = kEarthTileSize / metersPerPixel;
    float kTileResolution = 256.0;
    float tiles = pixels / kTileResolution;
    return log2(tiles) - 1.0;
}
uniform float width;
...
void main()
{
    ...
    vec4 tanPoint =
        ...
        upScalePoint(position + abs(sPosition.w / 10.0) * vec4(vec3(tangent), 0.0), modelMatrix);
    vec2 pTangent = normalize(sPosition.xy / sPosition.w - tanPoint.xy / tanPoint.w);
    mat2 rotate = mat2(pTangent.x, pTangent.y, -pTangent.y, pTangent.x);
    vec4 vTan = modelViewMatrix * normalize(vec4(vec3(tangent), 0.0));
    vec4 vPos = modelViewMatrix * scaledPosition;
    vec3 vTanDir = normalize(vTan.xyz);
    vec3 vPosDir = normalize(vPos.xyz);
    float vProjection = dot(vTanDir, vPosDir);
    float coef = sqrt(1.0 - vProjection * vProjection);
    float depth = abs(vPos.z);
    float zoomLevel = calculateZoomLevel(depth / coef, 1.0 / pixelSize.y, projectionMatrix);
    float instanceId = secondary_colour;
    int zoomLevelI = int(zoomLevel);
    float levelDiff = float(iconsZoomLevel - zoomLevelI);
    levelDiff = clamp(levelDiff, 0.0, float(iconsZoomLevel));
    float N = pow(2.0, levelDiff);
    bool shouldDisplay = int(mod(instanceId, N)) == 0;
    vOpacity = shouldDisplay ? vOpacity : 0.0;
    sPosition.xy +=
        ((
             rotate *
