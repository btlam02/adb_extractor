...
float getProjectionOnXYPlaneDirection(vec3 vector) { return atan(-vector.x, -vector.y); }
float getProjectionOnXYPlaneNormalizedDirection(vec3 vector)
{
    return (getProjectionOnXYPlaneDirection(vector) + PI) / doublePI;
}
void main()
{
    ...
    vec2 gPos;
    float normalizedDirection = getProjectionOnXYPlaneNormalizedDirection(textureNormal);
    float normalizedAngle = getNormalizedAngleBetweenAxisZ(textureNormal);
    gPos = vec2(normalizedAngle / 128.0 * 90.0, normalizedDirection);
