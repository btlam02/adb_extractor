...
uniform vec2 gridSnapPoint;
uniform float gridOpacity;
uniform float gridSharpness;
vec2 _round(vec2 value) { return floor(value + 0.5); }
vec4 drawGrid(vec2 uv, float distanceAlpha)
{
    vec2 pos = vPosition.xy - gridSnapPoint;
    vec2 _uv = abs(pos);
    _uv = abs(_uv - _round(_uv.xy / gridScale) * gridScale);
    float dist = min(_uv.x, _uv.y);
    float K = abs(vDepth) * gridThickness;
    if (dist < K)
    {
        float lineAlpha = 1.0 - dist / K;
        lineAlpha = mix(0.0, 1.0, clamp(lineAlpha * gridSharpness, 0.0, 1.0));
        float alpha = clamp(lineAlpha * distanceAlpha, 0.0, 1.0);
        return vec4(gridColor, alpha * gridOpacity);
    }
    return vec4(0.0);
}
void main()
{
    ...
    overlayColor = drawGrid(vUv, vDistanceAlpha);
    overlayColor =
