...
varying float vUV_yScaled;
float calculateDashFactor(float lineDash, float lineDashGap, float lineWidth)
{
    float dashPatternLength = (lineDash + lineDashGap);
    float depth = log2(max(1e-6, vDistance * lineWidth));
    float logDepth = pow(2.0, floor(depth));
    float uvy = vUV_yScaled / logDepth;
    uvy += (dashPatternLength * 0.5);
    float normalizedPatternPosition = fract(uvy / dashPatternLength);
    return step(lineDashGap / dashPatternLength, normalizedPatternPosition);
}
                    ...
                    float lineDashGap, float opacityMultiplier)
{
    ...
    color.a *= lineFactor;
    float lineDashFactor = calculateDashFactor(lineDash, lineDashGap, lineWidth);
    color.a *= lineDashFactor;
    return color;
}
